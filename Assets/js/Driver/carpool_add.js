...
// Presque sûr que l'échec lors de la première tentative venait de l'oubli de l'écouteur sur le 'change' de l'input en fin de la fonction initAddressSuggestions() dans search.js.
...
let calculDurationTimerId;          // ID du timeout pour calculer la durée du voyage
const proposeTimer = 1000 ;         // Délai en millisecondes avant de proposer des adresses
const geocodeTimer = 2000 ;         // Délai en millisecondes avant de géocoder une adresse
const calculDurationTimer = 500 ;   // Délai en millisecondes avant de calculer la durée du voyage
...
function initAddressSuggestions(input, suggestionContainer, geocodeObject) {
    ...
    input.addEventListener("keyup", () => {
        ...
        geocodeAddress(input, geocodeObject, geocodeTimer, checkAndCalculateDuration);
    });
...
function checkAndCalculateDuration() {
    ...
    && depDateAddCarpool.value !== '') {
        durationCalculated(depGeocode, arrGeocode, calculDurationTimer);
    }
...
function durationCalculated(depGeocodeObject, arrGeocodeObject, timer){
    clearTimeout(calculDurationTimerId);

    calculDurationTimerId = setTimeout(() => {
        const body = {
...
//// NON FONCTIONNNEL avec '../Tools/geoplateforme.js'
// import { apiUrl, getInfoUser, getToken } from '../script.js';
// import { geocodeAddress, proposeAddress }from '../Tools/geoplateforme.js';
// import { checkIfInputNonEmpty, dateToDatetimeLocal, escapeHTML }from '../Tools/tools.js';
...
//     // Vérification du formulaire saisi
//     btnAddCarpool.disabled = true;
//     depDateAddCarpool.addEventListener("change", checkInputs);
//     depAddressAddCarpool.addEventListener("keyup", checkInputs);
//     arrAddressAddCarpool.addEventListener("keyup", checkInputs);
//     arrDateAddCarpool.addEventListener("change", checkInputs);
//     priceAddCarpool.addEventListener("keyup", checkInputs);
//     nbPlaceAddCarpool.addEventListener("keyup", checkInputs);
...
// /**
//  * Récupère la liste de véhicules à l'aide de getListFromAPI et charge le HTML de saisi de covoiturage en conséquence.
//  *
//  * @async
//  */
// async function loadCarpoolAdd() {
...
//  */
// function initAddressSuggestions(input, suggestionContainer, geocodeObject) {
//     let proposeTimerId = null;
//     let geocodeTimerId = null;

//     input.addEventListener("keyup", () => {
//         proposeTimerId = proposeAddress(input, suggestionContainer, proposeTimer, proposeTimerId);
//         geocodeTimerId = geocodeAddress(input, geocodeObject, geocodeTimer, geocodeTimerId, checkAndCalculateDuration);
//     });
//     // A la reprise du focus si des suggestions existent déjà
//     input.addEventListener('focus', () => {
//         if (suggestionContainer.innerHTML.trim() !== '') {
